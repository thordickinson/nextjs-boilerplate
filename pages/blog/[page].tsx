...
export default function PostList({ posts, tags, recent }) {
  ...
  return (
    <DefaultLayout>
      <PageHeader
        sectionTitle="Blog"
        subtitle="This is the subtitle of the blog"
      ></PageHeader>
      <AsideContentSection aside={asideContent}>
        <div className={styles.postList}>
          {posts.content.map((p) => (
            <div key={p._id} className={styles.entryCard}>
...
export async function getStaticProps(
  ...
  const res = await fetch(
    `${process.env.BACKEND_URL}/api/blog/posts?limit=6&sort=-createdAt&page=${page}`
  );
  ...
  const recentResponse = await fetch(
    `${process.env.BACKEND_URL}/api/blog/posts?sort=-creacion&limit=4`
  );
